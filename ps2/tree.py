...
def compute_MI(A, C, i, j):
    ...
    for c in range(2):
        ...
        term1 = np.array([p_joint[idx] for idx in idxs])
        term2 = np.log(np.array([p_cond_i[idx[0]] for idx in idxs]))
        term3 = np.log(np.array([p_cond_j[idx[1]] for idx in idxs]))
        term4 = np.log(np.array([p_joint[idx] for idx in idxs]))

        _I_ij = p_c * sum(term1 * term4 - (term2 + term3))
