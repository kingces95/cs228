...
class FactorGraph:
    ...
    def runParallelLoopyBP(self, iterations):
        ...
        for i in range(iterations):
            ...
            for factorId in range(numFactors):
                ...
                # for each variable adjacent to this factor, send a message
                for recipientVarId in self.factorToVar[factorId]:

                    # multiply factor with all inbound messages (except the recipient)
                    product=factor

                    # for each variable adjacent to this factor (except the recipient)
                    for varId in self.factorToVar[factorId]:

                        if varId == recipientVarId:
                            continue

                        # receive message from variable to this factor
                        message=self.messagesVarToFactor[(varId, factorId)]

                        # accumulate messages to sent this factor
                        product=product.multiply(message.normalize())

                    # pivot on variable
                    message=product.marginalize_all_but([recipientVarId])

                    # send message
                    self.messagesFactorToVar[(factorId, recipientVarId)]=message
            ...
            for varId in range(numVar):
                ...
                # for each factor adjacent to this variable, send a message
                for recipientFactorId in self.varToFactor[varId]:

                    # accumulate a product of all inbound messages (except the recipient)
                    product=None

                    # for each factor adjacent to this variable (except the recipient)
                    for factorId in self.varToFactor[varId]:
                        ...
                        product=message.normalize() if product == None else product.multiply(message.normalize())

                    # send message
                    self.messagesVarToFactor[(varId, recipientFactorId)]=message
    ...
    def estimateMarginalProbability(self, varId):
        ...
        # array to hold latest message from each adjacent factor
        inbound=[]
